<?php
namespace SeveAlex\MobilPay\Request;

/**
 * mobilPay
 *
 * @package   Mobilpay_Payment_Request_Card
...
class Mobilpay_Payment_Request_Card extends Mobilpay_Payment_Request_Abstract {
     ...
     * @param DOMNode $elem
     * @return Mobilpay_Payment_Reuquest_Card
	...
	if ($elems->length > 0) {
	    $this->split = new Mobilpay_Payment_Split($elems->item(0));
	}
	
	$elems = $elem->getElementsByTagName('payment_instrument');
	if ($elems->length > 0) {
	    $this->paymentInstrument = new Mobilpay_Payment_Instrument_Card($elems->item(0));
	}
	return $this;
    }
	...
	if ($this->split instanceof Mobilpay_Payment_Split) {
